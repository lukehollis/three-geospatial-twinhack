...
export function useCloudsControls(
  ...
  // We're using the coverage value directly from props and not creating a control for it
  // This allows external control via the cloudCoverageAtom
  const { enabled, animate, qualityPreset } = useControls('clouds', {
    enabled: true,
